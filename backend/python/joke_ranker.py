...
class JokeRanker:
    ...
    def rank_jokes(self, query, top_n=5, return_latent=False):
        ...
        if return_latent:
            joke_latents = [self.joke_reduced[i] for i in ranked_indices]
            return [(self.jokes[i], similarities[i], joke_latents[j]) for j, i in enumerate(ranked_indices)]
